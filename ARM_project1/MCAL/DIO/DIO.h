...
*----------------------------------------------------
*			File: DIO.h
*			Description: Header for DIO containing Types, 
*			functions Prototypes and user interface Macos
*
*----------------------------------------------------*/
...
typedef struct{
	 ...
	 uint8  Port_Value; // which port to choose
...
enum INT_Sense { EDGE  = 0,
								 LEVEL = 1};

enum INT_Both_Edges { ONLY_ONE = 0 ,
											BOTH_EDGES_TRIGER = 1};

enum INT_Event { Falling_LOW = 0 ,
								 Rising_HIGH = 1 };
