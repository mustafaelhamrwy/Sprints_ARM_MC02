...
/*------------------NOTES----------------------*
 ...
 Remmeber Port c in pinconfig doesb't work!! Check it 
...
void Port_Init(Port_Config *configPtr){
	...
	int x ;
		//RCGCGPIO = 0xFF;
	SET_BIT(RCGCGPIO,configPtr->Port_Value );
	for(x=0;x<=3;x++){
	}
		//*again Function select is active love to GPIO function but here i did it respond to high*/
	 if( (configPtr->Port_PinModeType) == STD_HIGH){ 
		 GPIOAFSEL(configPtr->Port_Value) = 0x00 ; 
	 }		 
	 else {																				
		 GPIOAFSEL(configPtr->Port_Value) = 0xFF ;
	 }
	
	 if( (configPtr->Port_PinDirectionType) == STD_HIGH) { GPIODIR(configPtr->Port_Value) = 0xFF ;}
	 else{ 																							
		 GPIODIR(configPtr->Port_Value) = 0x00;
	 }
	  
	 if( (configPtr->Port_PinInternalAttachType) == STD_HIGH){
	 GPIOPUR(configPtr->Port_Value) = 0xFF ;
	 }
	 else{							
		 GPIOPDR(configPtr->Port_Value) = 0xFF ;
	 }
	 ...
	 if( (configPtr->Port_PinType) == STD_HIGH){
	 GPIOODR(configPtr->Port_Value) = 0xFF ;}
	 else{  																		
		 GPIOODR(configPtr->Port_Value) = 0x00 ;
	 }
